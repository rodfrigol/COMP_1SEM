...
TEST_F(OakTests, OakByNorm) {
    ...
    for (int step = 1; step <= step_number ; step++) {
        ...
        float t_ins_stl = tick.tickMili(); // time for stepsize insertions
        next = old_next;
        // add stepsize elements
        tick.tickMili();
        for (long i = 0 ; (i < stepsize) && (next < oak.size()) ; i++) {
        ...
        float t_ins_aluno = tick.tickMili();// time for stepsize insertions
        ...
        tick.tickMili();
        idx->find(res, oak[randomptidx].norm()); // look for the randompoint
        float timems = tick.tickMili();// time for one search
        idxaluno->find(resaluno, oak[randomptidx].norm()); // look for the randompoint
        float timemsaluno = tick.tickMili(); // time for one search
        result_dump << step << ", "<< next << ", "<< timems << ", " << timemsaluno << ", " << t_ins_stl << ", " << t_ins_aluno << std::endl; 
        ASSERT_NEAR(oak[res[0]].norm(),oak[randomptidx].norm(),MILI); // assert we found the same norm
        ASSERT_NEAR(oak[resaluno[0]].norm(),oak[randomptidx].norm(),MILI);
...
TEST_F(OakTests, VoidSphereSelection) {
    ...
    std::cout << " time (ms) MMP: " << timems << " aluno: " << timemsaluno << std::endl;
